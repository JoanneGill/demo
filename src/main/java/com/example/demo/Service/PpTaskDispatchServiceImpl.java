...
public class PpTaskDispatchServiceImpl implements PpTaskDispatchService {
    ...
    public PpTask addPpTask(PpTask ppTask) {
        ...
        // ========== 1. 参数校验 ==========
        if (ppTask.getNumber() == null || ppTask.getNumber() <= 0) {
        ...
        // 去除可能的首尾空白（用户粘贴地址时常带空格）
        ppTask.setPersonAddress(ppTask.getPersonAddress().trim());
        ...
        if (!jsonElement.has("roomId") || jsonElement.get("roomId").isJsonNull()) {
            throw new IllegalArgumentException("主播当前未开播");
        }
        ...
        ppTask.setNumberLeft(ppTask.getNumber());
        ...
        log.info("新增 ppTask 成功: id={}, roomId={}, personName={}, number={}",
                ppTask.getId(), roomId, personName, ppTask.getNumber());
